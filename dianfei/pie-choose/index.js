...
module.exports = Event.extend(function Base(container, config) {
  ...
  render: function (data, config) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (i < 2) {
        curDom.classList.add('item-pie');
        curDom.classList.add(`item-pie-${i + 1}`);
        ...
        curDom.classList.add('chart-pie');
        chartDom = curDom;
      }
    ...
    // let domList = document.querySelectorAll('.item-pie');
  ...
  optionData(data, config) {
    ...
    return {
      ...
      series: [{
        ...
        center: ['35%', '50%'],
        color: ['#0FCEB4', '#F3D200', '#00AFFD', '#FA4A79', '#4A63F0', '#FF48F0', '#FF4873', '#fff77f'],
        data: data,
        labelLine: {
          normal: {
            show: false,
